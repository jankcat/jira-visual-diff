...
namespace Jankcat.VisualCompare.Lib.SeleniumSupport
{
    public static class SeleniumUtils
    {
        ...
        public static void NormalScroll(IWebDriver driver, int x, int y)
        {
            ((IJavaScriptExecutor)driver).ExecuteScript(String.Format("window.scrollTo({0}, {1});", x, y));
        }

        public static void VirtualScroll(IWebDriver driver, int x, int y, bool reset = false)
        {
        ...
        public static Dimensions GetPageDimensions(IWebDriver driver)
        {
            // Get Page Height
        ...
        public static IMagickImage MakeAreaScreenshot(IWebDriver driver, int startX, int startY, int endX, int endY, ScreenshotOptions options)
        {
